 ...
 * （5）front、rear等于maxSize后重置为0
 * （6）队列已满时：isLoop == true && front == rear
 * （7）有效个数：若 isLoop == true，则有 maxSize - (front - rear)、若 isLoop == false，则有 rear - front
 */
...
//        LoopArrayCreateQueue_2 queue = new LoopArrayCreateQueue_2(5);
        ...
        while (true) {
            System.out.println("请输入操作，1：入队、2：出队、3：打印");
            int i = s.nextInt();
            switch (i) {
                case 1:
...
class LoopArrayCreateQueue_1 {
    ...
    public boolean enq(int e) {
        ...
        arr[rear] = e;  // 元素入队
        rear = (rear + 1) % maxSize;  // 入队下标后移，准备下一个入队元素的位置
        return true;
    }
...
class LoopArrayCreateQueue_2 {
     ...
     */
    public int size() {
        return isLoop ? maxSize - (front - rear) : rear - front;
    }
